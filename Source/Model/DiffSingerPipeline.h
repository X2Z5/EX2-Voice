...
public:
                                ...
                                const RenderOptions& ro)
    {
        ...
        try
        {
            ...
            int64_t speedupVal = v.cfgSpeedup, depthVal = v.cfgDepth, stepsVal = v.cfgSteps;
            float   tStartVal  = 0.0f;
            ...
            for (size_t i = 0; i < nIn; ++i)
            {
                ...
                {
                    auto& val = (nm == "speedup") ? speedupVal
                              : (nm == "depth")   ? depthVal : stepsVal;
                    inNames.push_back (nm);
                    if (dims.empty())
                ...
                else if (nm == "t_start")              // reflow models: diffusion start
                {
                    inNames.push_back (nm);
                    if (dims.empty())
                        inVals.push_back (Ort::Value::CreateTensor<float> (mem, &tStartVal, 1, nullptr, 0));
                    else
                    {
                        static const int64_t one[1] = { 1 };
                        inVals.push_back (Ort::Value::CreateTensor<float> (mem, &tStartVal, 1, one, 1));
                    }
                }
